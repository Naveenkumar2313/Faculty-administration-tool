...
import {
    ...
    Collapse, Grid, Alert, Divider
} from "@mui/material";
import {
    CheckCircle, Cancel, HelpOutline, Search, Download, Refresh, AccessTime, Warning,
    BarChart, ChevronLeft, ChevronRight, ExpandMore, ExpandLess, Visibility, Send, MeetingRoom
...
/* ── Mock Requests ── */
const REQUESTS = [
    { id: 1, requester: "Dr. Sarah Smith", avatar: "SS", dept: "CSE", room: "Conference Room A", purpose: "Department Meeting", date: "2026-04-10", time: "10:00 – 11:30", attendees: 15, status: "Pending", applied: "2026-04-05", conflict: true, notes: "Requires projector and whiteboard." },
    { id: 2, requester: "Prof. Rajan Kumar", avatar: "RK", dept: "Mech", room: "Meeting Room 102", purpose: "Project Review", date: "2026-04-12", time: "14:00 – 15:00", attendees: 8, status: "Pending", applied: "2026-04-06", conflict: false, notes: "" },
    { id: 3, requester: "Student Council", avatar: "SC", dept: "General", room: "Board Room 1", purpose: "Student Welfare Meeting", date: "2026-04-10", time: "10:30 – 12:00", attendees: 20, status: "Pending", applied: "2026-04-07", conflict: true, notes: "Video conferencing setup needed." },
    { id: 4, requester: "Dr. Emily Davis", avatar: "ED", dept: "CSE", room: "Meeting Room 201", purpose: "Research Discussion", date: "2026-04-15", time: "09:00 – 10:00", attendees: 5, status: "Approved", applied: "2026-04-03", conflict: false, notes: "" },
    { id: 5, requester: "Prof. A. Sharma", avatar: "AS", dept: "ECE", room: "Conference Room B", purpose: "Faculty Orientation", date: "2026-04-18", time: "11:00 – 13:00", attendees: 30, status: "Rejected", applied: "2026-04-02", conflict: false, notes: "Conflicted with a scheduled exam." },
];
...
const RoomBooking = () => {
    ...
    const filtered = REQUESTS.filter(r => {
        ...
        if (searchQ && !r.requester.toLowerCase().includes(searchQ.toLowerCase()) && !r.room.toLowerCase().includes(searchQ.toLowerCase())) return false;
        return true;
    });
    ...
    return (
        <Box sx={{ p: 3, background: T.bg, minHeight: "100vh", fontFamily: fontBody }}>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={3} className="fade-up">
                <Box>
                    <Typography sx={{ fontFamily: fontHead, fontWeight: 700, fontSize: "1.5rem", color: T.text, lineHeight: 1.1 }}>Room Booking Requests</Typography>
                    <Typography sx={{ fontFamily: fontBody, fontSize: "0.82rem", color: T.textSub, mt: 0.4 }}>
                        Review and process room reservation requests from faculty &amp; students
                    </Typography>
                </Box>
            ...
            <Box display="flex" gap={2} mb={3} sx={{ overflowX: "auto", pb: 0.5 }}>
                ...
                <StatCard label="Conflict Alerts" value={conflictCount} sub="Overlapping time slots" color={T.danger} icon={Warning} />
                <StatCard label="Total Requests" value={REQUESTS.length} sub="All statuses" color={T.accent} icon={MeetingRoom} />
            </Box>
            ...
            <Card sx={{ borderRadius: "16px", border: `1px solid ${T.border}`, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
                <Box sx={{ p: 2.5, borderBottom: `1px solid ${T.border}`, background: T.surface }}>
                    ...
                    <Box display="flex" gap={1.5} alignItems="center" flexWrap="wrap">
                        <TextField size="small" placeholder="Search requester or room..." value={searchQ} onChange={e => setSearchQ(e.target.value)}
                            InputProps={{ startAdornment: <InputAdornment position="start"><Search sx={{ fontSize: 16, color: T.textMute }} /></InputAdornment> }}
                            sx={{ width: 260, "& .MuiOutlinedInput-root": { borderRadius: "8px", fontFamily: fontBody, fontSize: "0.82rem", "& fieldset": { borderColor: T.border }, "&:hover fieldset": { borderColor: T.accent }, "&.Mui-focused fieldset": { borderColor: T.accent } } }}
                        />
                        {[
                            { label: "Department", value: deptFilter, set: setDeptFilter, options: ["All", "CSE", "Mech", "ECE", "General"] },
                    ...
                    {conflictCount > 0 && (
                        <Alert severity="warning" icon={<Warning sx={{ fontSize: 18 }} />}
                            sx={{ mt: 2, borderRadius: "10px", fontFamily: fontBody, fontSize: "0.8rem", border: `1px solid ${T.warning}40`, bgcolor: T.warningLight }}>
                            <strong>{conflictCount} conflict(s) detected:</strong> Some rooms have overlapping booking requests on the same date. Review before approving.
                        </Alert>
                    )}
                </Box>
                    ...
                    <Table size="medium">
                        <TableHead>
                            <TableRow sx={{ bgcolor: "#F9FAFB" }}>
                                {["Requester & Dept", "Room", "Purpose", "Date & Time", "Attendees", "Status", "Actions"].map(h => (
                                    ...
                                    <TableRow className="row-hover" sx={{ borderBottom: expandRow === row.id ? "none" : `1px solid ${T.border}` }}>
                                                ...
                                                <Box>
                                                    ...
                                                    <Box display="flex" alignItems="center" gap={0.6} mt={0.2}>
                                                        ...
                                                        {row.conflict && row.status === "Pending" && (
                                                            <Tooltip title="Conflict: Overlapping booking on same date/room">
                                        ...
                                        <TableCell>
                                            <Box sx={{ px: 1.2, py: 0.35, borderRadius: "6px", bgcolor: T.purpleLight, border: `1px solid ${T.purple}30`, display: "inline-block" }}>
                                                <Typography sx={{ fontFamily: fontMono, fontSize: "0.7rem", fontWeight: 600, color: T.purple }}>{row.room}</Typography>
                                            </Box>
                                        </TableCell>
                                        {/* Purpose */}
                                        <TableCell>
                                            <Typography sx={{ fontFamily: fontBody, fontSize: "0.8rem", color: T.textSub, maxWidth: 160, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{row.purpose}</Typography>
                                        </TableCell>
                                        {/* Date & Time */}
                                        <TableCell>
                                        ...
                                        {/* Attendees */}
                                        <TableCell>
                                            <Typography sx={{ fontFamily: fontMono, fontSize: "0.8rem", color: T.textSub }}>{row.attendees}</Typography>
                                        </TableCell>
                                        {/* Status */}
                                            ...
                                            <Collapse in={expandRow === row.id} timeout="auto" unmountOnExit>
                                                <Box sx={{ px: 3, py: 2.5, bgcolor: "#FAFBFD", borderBottom: `1px solid ${T.border}`, borderTop: `1px dashed ${T.border}` }}>
                                                    <Grid container spacing={3}>
                                                        <Grid item xs={12} md={4}>
                                                            ...
                                                            <Box display="flex" flexDirection="column" gap={1} mt={1}>
                                                                {[
                                                                    ...
                                                                    { k: "Attendees", v: `${row.attendees} persons` },
                                                                ].map(item => (
                                                        ...
                                                        <Grid item xs={12} md={4}>
                                                            <SLabel>Purpose / Notes</SLabel>
                                                            <Box mt={1} p={1.5} sx={{ bgcolor: T.surface, borderRadius: "8px", border: `1px solid ${T.border}` }}>
                                                                <Typography sx={{ fontFamily: fontBody, fontSize: "0.78rem", color: T.textSub }}>{row.purpose}</Typography>
                                                                {row.notes && <Typography sx={{ fontFamily: fontBody, fontSize: "0.75rem", color: T.textMute, mt: 1, fontStyle: "italic" }}>{row.notes}</Typography>}
                                                            </Box>
                                                        </Grid>
                                                        <Grid item xs={12} md={4}>
                                                            {row.conflict && (
                                                                <Box p={1.5} sx={{ bgcolor: T.dangerLight, borderRadius: "8px", border: `1px solid ${T.danger}30` }}>
                                                                    <Typography sx={{ fontFamily: fontBody, fontSize: "0.75rem", color: T.danger, fontWeight: 600 }}>
                                                                        ⚠ Conflict: Another booking request overlaps for the same room/date. Review before approving.
                                                                    </Typography>
                                                                </Box>
                                                            )}
                                                        </Grid>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} PaperProps={{ sx: { borderRadius: "16px", width: 460, border: `1px solid ${T.border}`, boxShadow: "0 20px 60px rgba(0,0,0,0.12)" } }}>
                <DialogTitle sx={{ fontFamily: fontHead, fontWeight: 700, fontSize: "1rem", color: T.text, pb: 1, pt: 3, px: 3 }}>
                    {dialogData?.action === "approve" ? "Approve Room Request" : dialogData?.action === "reject" ? "Reject Room Request" : "Request Clarification"}
                </DialogTitle>
                        ...
                        <Box>
                            <Box sx={{ p: 2, borderRadius: "10px", bgcolor: T.bg, border: `1px solid ${T.border}`, mb: 2 }}>
                                ...
                                <Typography sx={{ fontFamily: fontBody, fontSize: "0.75rem", color: T.textMute, mt: 0.3 }}>
                                    {dialogData.row.room} · {dialogData.row.date} · {dialogData.row.time}
                                </Typography>
                            </Box>
                            {dialogData.action === "approve" && dialogData.row.conflict && (
                                <Alert severity="warning" sx={{ mb: 2, borderRadius: "10px", fontFamily: fontBody, fontSize: "0.78rem" }}>
                                    Approving this creates a scheduling conflict on {dialogData.row.date}.
                                </Alert>
                            )}
                ...
                <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
                    <Button onClick={() => setDialogOpen(false)} variant="outlined"
                        sx={{ fontFamily: fontBody, fontWeight: 600, fontSize: "0.8rem", textTransform: "none", borderRadius: "8px", borderColor: T.border, color: T.textSub }}>Cancel</Button>
                    <Button variant="contained"
                        sx={{
                            fontFamily: fontBody, fontWeight: 700, fontSize: "0.8rem", textTransform: "none", borderRadius: "8px", boxShadow: "none",
