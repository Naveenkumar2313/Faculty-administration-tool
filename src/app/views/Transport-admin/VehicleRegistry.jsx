...
import {
    Search, Add, DirectionsBus, Edit, Close, Visibility,
    LocalGasStation, Speed, Build, Map, KeyboardArrowUp, KeyboardArrowDown
} from "@mui/icons-material";
import { T, fontHead, fontBody, fontMono, Fonts, buses, statusColors } from "./transportAdminShared";
        ...
        <Box sx={{ p: 3, background: T.bg, minHeight: "100vh", fontFamily: fontBody }}>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={3} className="fade-up">
                <Box>
                    ...
                    <Typography sx={{ fontFamily: fontHead, fontWeight: 700, fontSize: "1.5rem", color: T.text, lineHeight: 1.1 }}>Vehicle Registry</Typography>
                    <Typography sx={{ fontFamily: fontBody, fontSize: "0.82rem", color: T.textSub, mt: 0.4 }}>All buses — registration, insurance, fitness certificates, and service due dates.</Typography>
                </Box>
            ...
            <Card className="fade-up" sx={{ borderRadius: "14px", border: `1px solid ${T.border}`, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                <Box sx={{ p: 2.5, borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 2 }}>
                    <Box>
                        <Typography sx={{ fontFamily: fontHead, fontWeight: 700, fontSize: "1.1rem" }}>Vehicle Registry</Typography>
                    ...
                    <Table>
                        <TableHead>
                            <TableRow sx={{ bgcolor: "#F9FAFB" }}>
                                {["Bus ID / Number", "Make & Year", "Driver", "Route", "Capacity", "Fuel", "Insurance", "Status", "Actions"].map(h => (
                        ...
                        <TableBody>
                            {filtered.map(row => (
                                <TableRow key={row.id} className="row-hover" sx={{ cursor: "pointer" }} onClick={() => setDetailBus(row)}>
                                    ...
                                    <TableCell><Typography sx={{ fontFamily: fontBody, fontSize: "0.8rem", color: T.textSub, maxWidth: 180 }} noWrap>{row.route}</Typography></TableCell>
                                    <TableCell>
                                        <Box>
                                            <Typography sx={{ fontFamily: fontMono, fontSize: "0.78rem", color: T.text, fontWeight: 600 }}>{row.occupancy}/{row.capacity}</Typography>
                                            <LinearProgress variant="determinate" value={(row.occupancy / row.capacity) * 100}
                                    ...
                                    <TableCell sx={{ fontFamily: fontBody, fontSize: "0.75rem", color: T.textSub }}>{row.insuranceExpiry}</TableCell>
                    ...
                    <DialogContent dividers>
                        <Grid container spacing={2}>
                            {[
                                ...
                                { label: "Fitness Expiry", value: detailBus.fitnessExpiry },
                                { label: "GPS Device", value: detailBus.gpsDeviceId },
