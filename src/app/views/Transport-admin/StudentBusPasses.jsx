...
import {
    ...
    Close, Visibility, Refresh, FileDownload
} from "@mui/icons-material";
import { T, fontHead, fontBody, fontMono, Fonts, students, statusColors } from "./transportAdminShared";
...
export default function StudentBusPasses() {
        ...
        <Box sx={{ p: 3, background: T.bg, minHeight: "100vh", fontFamily: fontBody }}>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={3} className="fade-up">
                <Box>
                    ...
                    <Typography sx={{ fontFamily: fontBody, fontSize: "0.82rem", color: T.textSub, mt: 0.4 }}>Approve, issue, and renew transport passes for students.</Typography>
                </Box>
                        ...
                        <TableBody>
                            {filtered.slice(0, 25).map(row => (
                                <TableRow key={row.id} className="row-hover" sx={{ cursor: "pointer" }} onClick={() => setDetailStudent(row)}>
                                    ...
                                    <TableCell><Typography sx={{ fontFamily: fontBody, fontSize: "0.8rem", color: T.textSub, maxWidth: 160 }} noWrap>{row.route}</Typography></TableCell>
