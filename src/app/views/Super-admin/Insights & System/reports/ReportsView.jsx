...
import {
  ...
  Gavel, Close, Refresh, Search, DirectionsBus, LocalParking,
  Hotel, Build, Inventory2, Engineering
...
/* CIMS Domain Keys */
const CIMS_ZONES = ["Zone A", "Zone B", "Zone C", "Zone D"];
const CIMS_HOSTELS = ["Boys Hostel 1", "Boys Hostel 2", "Girls Hostel 1", "Girls Hostel 2"];
const CIMS_ROUTES = ["Route 1", "Route 2", "Route 3", "Route 4", "Route 5", "Route 6"];
const FACULTY_CATS = ["HR", "Financial", "Academic", "Compliance"];
...
/* ── CIMS Raw Data ── */
const CIMS_RAW = {
  transport: { buses: 10, routes: 6, drivers: 12, passes: 280, onTime: 87, fuelCost: 45.2, maintenance: 3, incidents: 2 },
  parking: { zones: 4, spots: 620, occupied: 478, passes: 340, revenue: 12.8, violations: 18, gateEntries: 1250, gateExits: 1180 },
  hostel: { blocks: 4, rooms: 480, students: 920, occupancy: 92, complaints: 34, resolved: 28, messRating: 3.8, attendance: 88 },
  maintenance: { requests: 156, open: 42, resolved: 114, avgDays: 3.2, staff: 28, critical: 8, workOrders: 89, overdue: 5 },
  assets: { equipment: 1240, furniture: 3400, inventory: 5800, working: 94, condemned: 3, underRepair: 3, totalValue: 42.5, audited: 78 },
};
...
const REPORTS = {
  ...
  /* ═══════════════════════════════════════
     CIMS REPORT CATEGORIES
  ═══════════════════════════════════════ */
  Transport: [
    {
      id: "tr_fleet", name: "Bus Fleet Utilisation Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Route", "Bus ID", "Capacity", "Avg Occupancy", "Utilisation %"],
      getData: (keys) => (keys.length ? keys : CIMS_ROUTES).map((r, i) => {
        const cap = 50, occ = Math.floor(Math.random() * 25) + 20, util = Math.round(occ / cap * 100);
        return { dept: r, vals: [r, `B-${String(i + 1).padStart(2, '0')}`, cap, occ, `${util}%`], chart: util };
      }),
      chartKey: "chart", chartLabel: "Utilisation %", chartMax: 100, chartColor: "#6366F1",
    },
    {
      id: "tr_pass", name: "Student Pass Issuance Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Route", "Active Passes", "Expired", "Pending Renewal", "Revenue (₹)"],
      getData: (keys) => (keys.length ? keys : CIMS_ROUTES).map((r, i) => {
        const active = Math.floor(Math.random() * 40) + 20, exp = Math.floor(Math.random() * 8), pending = Math.floor(Math.random() * 5);
        return { dept: r, vals: [r, active, exp, pending, `₹${(active * 10000).toLocaleString()}`], chart: active };
      }),
      chartKey: "chart", chartLabel: "Active Passes", chartMax: 70, chartColor: "#10B981",
    },
    {
      id: "tr_ontime", name: "On-Time Performance Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Route", "Total Trips", "On Time", "Delayed", "On-Time %"],
      getData: (keys) => (keys.length ? keys : CIMS_ROUTES).map((r) => {
        const total = Math.floor(Math.random() * 20) + 30, onTime = Math.floor(total * (0.78 + Math.random() * 0.2));
        return { dept: r, vals: [r, total, onTime, total - onTime, `${Math.round(onTime / total * 100)}%`], chart: Math.round(onTime / total * 100) };
      }),
      chartKey: "chart", chartLabel: "On-Time %", chartMax: 100, chartColor: "#F59E0B",
    },
  ],
  Parking: [
    {
      id: "pk_occ", name: "Zone Occupancy Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Zone", "Total Spots", "Occupied", "Available", "Occupancy %"],
      getData: (keys) => (keys.length ? keys : CIMS_ZONES).map((z, i) => {
        const total = Math.floor(Math.random() * 80) + 100, occ = Math.floor(total * (0.6 + Math.random() * 0.35));
        return { dept: z, vals: [z, total, occ, total - occ, `${Math.round(occ / total * 100)}%`], chart: Math.round(occ / total * 100) };
      }),
      chartKey: "chart", chartLabel: "Occupancy %", chartMax: 100, chartColor: "#6366F1",
    },
    {
      id: "pk_revenue", name: "Parking Revenue Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Zone", "Pass Revenue (₹L)", "Daily Revenue (₹)", "Fines (₹)", "Total (₹L)"],
      getData: (keys) => (keys.length ? keys : CIMS_ZONES).map((z) => {
        const pass = +(Math.random() * 3 + 1).toFixed(1), daily = Math.floor(Math.random() * 5000) + 2000, fines = Math.floor(Math.random() * 8000) + 500;
        return { dept: z, vals: [z, `₹${pass}L`, `₹${daily.toLocaleString()}`, `₹${fines.toLocaleString()}`, `₹${(pass + daily / 100000 + fines / 100000).toFixed(1)}L`], chart: pass };
      }),
      chartKey: "chart", chartLabel: "Revenue ₹L", chartMax: 5, chartColor: "#10B981",
    },
    {
      id: "pk_violations", name: "Violation & Incident Log", vizType: "table", icon: TableView,
      columns: ["Zone", "Unauthorized Parking", "Expired Pass", "Overstay", "Total Violations"],
      getData: (keys) => (keys.length ? keys : CIMS_ZONES).map((z) => {
        const unauth = Math.floor(Math.random() * 8) + 1, expired = Math.floor(Math.random() * 5), overstay = Math.floor(Math.random() * 6);
        return { dept: z, vals: [z, unauth, expired, overstay, unauth + expired + overstay], chart: unauth + expired + overstay };
      }),
      chartKey: "chart", chartLabel: "Violations", chartMax: 25, chartColor: "#EF4444",
    },
  ],
  Hostel: [
    {
      id: "hs_occ", name: "Room Occupancy Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Block", "Total Rooms", "Occupied", "Vacant", "Occupancy %"],
      getData: (keys) => (keys.length ? keys : CIMS_HOSTELS).map((h) => {
        const total = Math.floor(Math.random() * 40) + 100, occ = Math.floor(total * (0.8 + Math.random() * 0.18));
        return { dept: h, vals: [h, total, occ, total - occ, `${Math.round(occ / total * 100)}%`], chart: Math.round(occ / total * 100) };
      }),
      chartKey: "chart", chartLabel: "Occupancy %", chartMax: 100, chartColor: "#8B5CF6",
    },
    {
      id: "hs_complaint", name: "Complaint Resolution Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Block", "Total Complaints", "Resolved", "Pending", "Avg Days to Resolve"],
      getData: (keys) => (keys.length ? keys : CIMS_HOSTELS).map((h) => {
        const total = Math.floor(Math.random() * 15) + 5, resolved = Math.floor(total * (0.65 + Math.random() * 0.3));
        return { dept: h, vals: [h, total, resolved, total - resolved, `${(Math.random() * 3 + 1).toFixed(1)} days`], chart: resolved };
      }),
      chartKey: "chart", chartLabel: "Resolved", chartMax: 25, chartColor: "#10B981",
    },
    {
      id: "hs_mess", name: "Mess & Attendance Report", vizType: "table", icon: TableView,
      columns: ["Block", "Enrolled", "Avg Attendance %", "Mess Rating", "Wastage (kg/day)"],
      getData: (keys) => (keys.length ? keys : CIMS_HOSTELS).map((h) => {
        const enrolled = Math.floor(Math.random() * 100) + 150, att = Math.floor(Math.random() * 15) + 80;
        return { dept: h, vals: [h, enrolled, `${att}%`, (3 + Math.random() * 1.5).toFixed(1), Math.floor(Math.random() * 20) + 10], chart: att };
      }),
      chartKey: "chart", chartLabel: "Attendance %", chartMax: 100, chartColor: "#F59E0B",
    },
  ],
  Maintenance: [
    {
      id: "mn_req", name: "Request Volume & Resolution", vizType: "bar", icon: BarChartIcon,
      columns: ["Zone", "Total Requests", "Resolved", "Open", "Resolution Rate %"],
      getData: (keys) => (keys.length ? keys : CIMS_ZONES).map((z) => {
        const total = Math.floor(Math.random() * 30) + 15, resolved = Math.floor(total * (0.6 + Math.random() * 0.35));
        return { dept: z, vals: [z, total, resolved, total - resolved, `${Math.round(resolved / total * 100)}%`], chart: Math.round(resolved / total * 100) };
      }),
      chartKey: "chart", chartLabel: "Resolution %", chartMax: 100, chartColor: "#F59E0B",
    },
    {
      id: "mn_staff", name: "Staff Workload Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Zone", "Staff Assigned", "Avg Tasks/Staff", "Overdue Tasks", "Efficiency %"],
      getData: (keys) => (keys.length ? keys : CIMS_ZONES).map((z) => {
        const staff = Math.floor(Math.random() * 5) + 3, tasks = Math.floor(Math.random() * 4) + 3, overdue = Math.floor(Math.random() * 3);
        return { dept: z, vals: [z, staff, tasks, overdue, `${Math.floor(85 + Math.random() * 12)}%`], chart: staff * tasks };
      }),
      chartKey: "chart", chartLabel: "Tasks", chartMax: 40, chartColor: "#6366F1",
    },
    {
      id: "mn_cost", name: "Maintenance Cost Analysis", vizType: "table", icon: TableView,
      columns: ["Zone", "Labor Cost (₹)", "Material Cost (₹)", "External Cost (₹)", "Total (₹)"],
      getData: (keys) => (keys.length ? keys : CIMS_ZONES).map((z) => {
        const labor = Math.floor(Math.random() * 30000) + 15000, mat = Math.floor(Math.random() * 20000) + 8000, ext = Math.floor(Math.random() * 10000);
        return { dept: z, vals: [z, `₹${labor.toLocaleString()}`, `₹${mat.toLocaleString()}`, `₹${ext.toLocaleString()}`, `₹${(labor + mat + ext).toLocaleString()}`], chart: Math.round((labor + mat + ext) / 1000) };
      }),
      chartKey: "chart", chartLabel: "Cost (₹K)", chartMax: 80, chartColor: "#EF4444",
    },
  ],
  Assets: [
    {
      id: "as_inv", name: "Inventory Audit Summary", vizType: "bar", icon: BarChartIcon,
      columns: ["Category", "Total Items", "Working", "Under Repair", "Condemned"],
      getData: () => ["Electronics", "Lab Equipment", "Furniture", "Vehicles", "IT Hardware"].map((c) => {
        const total = Math.floor(Math.random() * 500) + 200, working = Math.floor(total * 0.9), repair = Math.floor(total * 0.06);
        return { dept: c, vals: [c, total, working, repair, total - working - repair], chart: total };
      }),
      chartKey: "chart", chartLabel: "Total Items", chartMax: 800, chartColor: "#14B8A6",
    },
    {
      id: "as_val", name: "Asset Valuation Report", vizType: "bar", icon: BarChartIcon,
      columns: ["Category", "Purchase Value (₹L)", "Current Value (₹L)", "Depreciation (₹L)", "Depreciation %"],
      getData: () => ["Electronics", "Lab Equipment", "Furniture", "Vehicles", "IT Hardware"].map((c) => {
        const purchase = +(Math.random() * 20 + 5).toFixed(1), current = +(purchase * (0.4 + Math.random() * 0.4)).toFixed(1);
        return { dept: c, vals: [c, `₹${purchase}L`, `₹${current}L`, `₹${(purchase - current).toFixed(1)}L`, `${Math.round((1 - current / purchase) * 100)}%`], chart: purchase };
      }),
      chartKey: "chart", chartLabel: "Value ₹L", chartMax: 30, chartColor: "#6366F1",
    },
    {
      id: "as_lifecycle", name: "Asset Lifecycle & Warranty", vizType: "table", icon: TableView,
      columns: ["Category", "Under Warranty", "Warranty Expired", "Due for Replacement", "Avg Age (yrs)"],
      getData: () => ["Electronics", "Lab Equipment", "Furniture", "Vehicles", "IT Hardware"].map((c) => {
        const warranty = Math.floor(Math.random() * 100) + 30, expired = Math.floor(Math.random() * 80) + 20, replace = Math.floor(Math.random() * 20) + 5;
        return { dept: c, vals: [c, warranty, expired, replace, `${(Math.random() * 4 + 1).toFixed(1)} yrs`], chart: warranty };
      }),
      chartKey: "chart", chartLabel: "Warranty Active", chartMax: 150, chartColor: "#10B981",
    },
  ],
};

const CAT_META = {
  HR: { color: T.accent, bg: T.accentLight, Icon: Assessment, domain: "faculty" },
  Financial: { color: T.success, bg: T.successLight, Icon: AccountBalance, domain: "faculty" },
  Academic: { color: T.purple, bg: T.purpleLight, Icon: School, domain: "faculty" },
  Compliance: { color: T.warning, bg: T.warningLight, Icon: Gavel, domain: "faculty" },
  Transport: { color: "#0891B2", bg: "#ECFEFF", Icon: DirectionsBus, domain: "cims" },
  Parking: { color: "#7C3AED", bg: "#F5F3FF", Icon: LocalParking, domain: "cims" },
  Hostel: { color: "#DB2777", bg: "#FDF2F8", Icon: Hotel, domain: "cims" },
  Maintenance: { color: "#D97706", bg: "#FEF3C7", Icon: Build, domain: "cims" },
  Assets: { color: "#059669", bg: "#ECFDF5", Icon: Inventory2, domain: "cims" },
};

const isCIMS = (cat) => CAT_META[cat]?.domain === "cims";
const getCIMSKeys = (cat) => {
  if (cat === "Transport") return CIMS_ROUTES;
  if (cat === "Parking" || cat === "Maintenance") return CIMS_ZONES;
  if (cat === "Hostel") return CIMS_HOSTELS;
  return [];
};
...
const TD = ({ children, sx = {}, align }) => (
  <TableCell align={align} sx={{
    fontFamily: fBody, fontSize: "0.8rem", color: T.textSub,
...
const ReportsView = () => {
  ...
  const switchCategory = (cat) => {
    ...
    setReportId("");
    setGenerated(false);
    /* Reset filter keys to match new domain */
    if (isCIMS(cat)) {
      setDepts([...getCIMSKeys(cat)]);
    } else {
      setDepts([...DEPARTMENTS]);
    }
  };
      ...
      <Box display="flex" justifyContent="space-between" alignItems="flex-start"
        ...
        <Box>
          <Typography sx={{ fontFamily: fBody, fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: T.textMute, mb: 0.3 }}>
            {isCIMS(category) ? "CIMS · Reports" : "Faculty Admin · Reports"}
          </Typography>
          ...
          <Typography sx={{ fontFamily: fBody, fontSize: "0.82rem", color: T.textSub, mt: 0.3 }}>
            Build, filter, preview, and export reports across Faculty Administration & CIMS domains.
          </Typography>
        </Box>
      </Box>
      ...
      <Grid container spacing={2.5} alignItems="flex-start">
        ...
        <Grid item xs={12} md={4}>
          <SCard sx={{ p: 2.5, position: "sticky", top: 24 }} className="fu">
            ...
            {/* Step 1 — Category */}
            <SLabel sx={{ mb: 0.6 }}>1 · Faculty Administration</SLabel>
            <Grid container spacing={1} mb={2.5}>
              {Object.keys(REPORTS).filter(c => !isCIMS(c)).map(cat => {
                ...
                return (
                  <Grid item xs={6} key={cat}>
                    <Box onClick={() => switchCategory(cat)}
                      ...
                      <Typography sx={{
                        fontFamily: fBody, fontWeight: active ? 700 : 600,
            ...
            <SLabel sx={{ mb: 0.6 }}>CIMS Infrastructure</SLabel>
            <Grid container spacing={1} mb={2.5}>
              {Object.keys(REPORTS).filter(c => isCIMS(c)).map(cat => {
                const m = CAT_META[cat];
                const active = category === cat;
                return (
                  <Grid item xs={6} key={cat}>
                    <Box onClick={() => switchCategory(cat)}
                      ...
                      <Typography sx={{
                        fontFamily: fBody, fontWeight: active ? 700 : 600,
                        fontSize: "0.74rem", color: active ? m.color : T.textSub
                      }}>
                        {cat}
                      </Typography>
                    </Box>
              ...
              {(REPORTS[category] || []).map((rep, i) => {
                ...
                return (
                  <Box key={rep.id}
                    ...
                    <Typography sx={{
                      fontFamily: fBody, fontSize: "0.77rem",
            ...
            <Stack spacing={1.8}>
              ...
              {/* Context-aware primary filter */}
              {!isCIMS(category) ? (
                /* Faculty: Departments multi-select */
                ...
                /* CIMS: Zone / Route / Block filter */
                <FormControl size="small" fullWidth sx={{
                  "& .MuiOutlinedInput-root": {
                    borderRadius: "8px", fontFamily: fBody, fontSize: "0.8rem",
                    "& fieldset": { borderColor: T.border },
                    "&.Mui-focused fieldset": { borderColor: catMeta.color }
                  },
                  "& .MuiInputLabel-root.Mui-focused": { color: catMeta.color }
                }}>
                  <InputLabel sx={{ fontFamily: fBody, fontSize: "0.8rem" }}>
                    {category === "Transport" ? "Routes" : category === "Hostel" ? "Blocks" : category === "Assets" ? "All Categories" : "Zones"}
                  </InputLabel>
                  <Select
                    multiple value={depts}
                    onChange={e => { setDepts(typeof e.target.value === "string" ? e.target.value.split(",") : e.target.value); setGenerated(false); }}
                    input={<OutlinedInput label={category === "Transport" ? "Routes" : category === "Hostel" ? "Blocks" : "Zones"} />}
                    renderValue={sel => {
                      const keys = getCIMSKeys(category);
                      return sel.length === keys.length || sel.length === 0 ? "All" : sel.join(", ");
                    }}
                  >
                    {getCIMSKeys(category).map(d => (
                      <MenuItem key={d} value={d} sx={{ fontFamily: fBody, fontSize: "0.8rem" }}>
                        <Checkbox checked={depts.includes(d)} size="small"
                          sx={{ "&.Mui-checked": { color: catMeta.color }, p: 0.5, mr: 0.5 }} />
                        <ListItemText primary={d} primaryTypographyProps={{ fontFamily: fBody, fontSize: "0.8rem" }} />
                      </MenuItem>
                    ))}
                  </Select>
              ...
              {/* Designation (only for Faculty reports) */}
              {!isCIMS(category) && (
                <Box>
            ...
            <Stack spacing={1}>
              <Button variant="contained" fullWidth size="large"
                ...
                sx={{
                  fontFamily: fBody, fontWeight: 700, fontSize: "0.85rem",
              ...
              {generated && (
                <Button variant="outlined" fullWidth size="small"
                  ...
                  onClick={handleReset}
                  sx={{
                    fontFamily: fBody, fontWeight: 600, fontSize: "0.76rem",
        ...
        <Grid item xs={12} md={8}>
          <SCard sx={{ overflow: "hidden", minHeight: 520, display: "flex", flexDirection: "column" }} className="fu1">
            ...
            <Box sx={{
              ...
              <Box>
                <Typography sx={{
                  fontFamily: fHead, fontWeight: 700,
              ...
              <Box display="flex" gap={1}>
                {[
                  ...
                  <Button key={e.fmt} size="small" variant="outlined"
                    ...
                    disabled={!generated}
                    sx={{
                      fontFamily: fBody, fontWeight: 600, fontSize: "0.72rem",
            ...
            <Box sx={{ p: 3, flex: 1 }}>
              ...
              {!generated && (
                <Box sx={{
                  ...
                  justifyContent: "center", color: T.textMute
                }} className="fu">
                  ...
                  <Typography sx={{
                    fontFamily: fBody, fontSize: "0.78rem",
                ...
                <Box className="fu">
                  ...
                  <Box display="flex" gap={1} flexWrap="wrap" mb={2.5}>
                    {[
                      ...
                      <Box key={i} sx={{
                        px: 1.2, py: 0.35, borderRadius: "99px",
                        bgcolor: c.bg, border: `1px solid ${c.color}25`
                      }}>
                        <Typography sx={{
                          fontFamily: fBody, fontSize: "0.7rem",
                  ...
                  <SCard sx={{ overflow: "hidden" }}>
                    ...
                    {previewRows.length > 0 && (
                      <Box sx={{
                        px: 2.5, py: 1.8, borderTop: `1px solid ${T.border}`,
