...
import { T, fontHead, fontBody, fontMono, Fonts, mockWorkOrders, statusColors } from "./maintenanceShared";
    ...
    return (
        <Box sx={{ p: 3, background: T.bg, minHeight: "100vh", fontFamily: fontBody }}>
            ...
            <Box mb={4} className="fade-up">
                ...
                <Typography sx={{ fontFamily: fontHead, fontWeight: 700, fontSize: "1.5rem", color: T.text, lineHeight: 1.1 }}>My Work Orders</Typography>
                <Typography sx={{ fontFamily: fontBody, fontSize: "0.85rem", color: T.textSub, mt: 0.4 }}>Review your currently assigned tasks.</Typography>
            </Box>
            ...
            <Card className="fade-up" sx={{ borderRadius: "14px", border: `1px solid ${T.border}`, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                <Box sx={{ p: 2.5, borderBottom: `1px solid ${T.border}` }}>
                    <Typography sx={{ fontFamily: fontHead, fontWeight: 700, fontSize: "1.1rem" }}>Open & In Progress Tasks</Typography>
                </Box>
                <Box sx={{ overflowX: "auto" }}>
                    <Table>
                        <TableHead>
                            <TableRow sx={{ bgcolor: T.bg }}>
                                {["Work ID", "Location", "Issue Description", "Priority", "Status", "Action"].map(h => (
                        ...
                        <TableBody>
                            {mockWorkOrders.filter(row => row.status !== "Completed").map(row => (
                                <TableRow key={row.id} className="row-hover">
                                    ...
                                    <TableCell sx={{ fontFamily: fontBody, fontSize: "0.85rem", color: T.textSub }}>{row.issue}</TableCell>
                                    <TableCell sx={{ fontFamily: fontMono, fontSize: "0.75rem", fontWeight: 700, color: row.priority === "High" ? T.error : T.textSub }}>{row.priority}</TableCell>
                                    <TableCell><StatusPill status={row.status} /></TableCell>
                                    <TableCell>
                                        <IconButton size="small" sx={{ color: T.info }}><Visibility fontSize="small" /></IconButton>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
